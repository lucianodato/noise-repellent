...
* **Low-Latency Mode**: Causal 1D-only path with a fixed 512-sample frame for live scenarios (~10.7 ms at 48 kHz / ~11.6 ms at 44.1 kHz). Enable it from the Options menu: the smoothing selector locks to Standard, the frame-size menu is disabled, and reduction/threshold links are forced on (the short frame is too coarse for independent tonal/broadband control). Toggling rebuilds the engine from a clean slate (profile dropped, Learn auto-stopped) and re-reports host delay compensation.
