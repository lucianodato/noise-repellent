...
* **Persistence Across States:** Once noise is learned (`learn_noise`), the noise profile is frozen and preserved across playback starts, stops, transport suspensions, and smoothing mode switches. Switching smoothing method (Temporal ↔ Patch-Based NLM ↔ NLM + DFTT Refinement ↔ Collaborative BM3D, internally 1D Spectral ↔ 2D NLM ↔ 2D NLM + DFTT ↔ BM3D) must never lose the profile or cause the visualizer to flatline to -120 dB.
...
* **No Host Suspension For Mode Switches:** Mode switching must never trigger `suspendProcessing`, latency re-reports, or profile migration in the plugin. Frame-size switches are the exception: they suspend, rebuild, and re-report. Low-latency mode is the second exception: it suspends, rebuilds (512-sample frame, causal 1D-only, zero look-ahead), re-reports (~10.7 ms at 48 kHz / ~11.6 ms at 44.1 kHz), and starts clean (profile dropped, Learn auto-stopped) like a frame-size switch. Entering it also installs control defaults (smoothing floor 30, aggressiveness 1, masking 0), adjustable afterwards. While active, the smoothing selector is locked to Standard and the frame-size menu is disabled. The 512-sample frame is too coarse for independent tonal/broadband control, so reduction and threshold-offset links are forced on (unlinking unavailable).
* **Low-Latency Smoothing Cap:** In low-latency mode the smoothing slider's top half maps to ~130 ms max release instead of 500 ms: the 512-sample frame's coarse LF bins smear gain envelopes into synth-pad artifacts at long releases.
