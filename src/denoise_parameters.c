...
float get_plugin_parameters(DenoiseParameters *self, int parameter_type)
{
    ...
    case WHITENING_FACTOR:
        return self->whitening_factor;
        break;
    case LEARN_NOISE:
        return (float)self->learn_noise;
        break;
    case RESIDUAL_LISTEN:
        return (float)self->residual_listen;
        break;
    case TRANSIENT_PROTECTION:
        return self->transient_threshold;
        break;
    case ENABLE:
        return (float)self->enable;
        break;
    }
...
void set_plugin_parameters(DenoiseParameters *self, float *value, int parameter_type)
{
    ...
    {
    case REDUCTION_AMOUNT:
        self->reduction_amount = FROM_DB_TO_CV(-1.f * *value);
        break;
    case NOISE_RESCALE:
    ...
    case MASKING:
        ...
        break;
    case WHITENING_FACTOR:
        self->whitening_factor = (*value / 100.f);
        break;
    case LEARN_NOISE:
        self->learn_noise = (bool)*value;
        break;
    case RESIDUAL_LISTEN:
        self->residual_listen = (bool)*value;
        break;
    case TRANSIENT_PROTECTION:
